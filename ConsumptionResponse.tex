...
In our primary scenario, we assume that this condition is removed with probability one-half after each quarter, so on average remains for two quarters.  When the `lockdown' ends, the buildup of savings by households who did not lose their jobs but whose spending was suppressed should result in a partial recovery in consumer spending, but in our primary scenario (without the CARES act), total consumer spending remains below its pre-crisis peak through the forseeable future.  % Added this here because without it the first sentence in the next paragraph was unclear about whether we were talking about the marginal effect of the CARES act or about a total projection of consumption taking everything into account.  Acknowledging the pent-up savings here, before we describe the stimulus, makes it clearer.  I have correspondingly shortened the subsequent para
...
First, households in our model who do not lose their jobs will initially build up their savings, both because of the `lockdown'-induced suppression of spending and because %for several reasons.
...
\begin{figure}
	...
	% \begin{Web}
	\ifthenelse{\boolean{ifWeb}}{
		\includegraphics[width=8in]{./Figures/LaborIncome_examples}
	} %\end{Web}
